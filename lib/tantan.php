...
class Amazon_S3_And_CloudFront {
    ...
    function delete_attachment( $post_id ) {
        ...
        delete_post_meta( $post_id, 'dreamspeed_info' );
    }

    function wp_update_attachment_metadata( $data, $post_id ) {
        if ( !$this->get_setting( 'copy-to-dreamspeed' ) || !$this->is_plugin_setup() ) {
            return $data;
        }
        ...
        add_post_meta( $post_id, 'dreamspeed_info', array(
	        'bucket' => $bucket,
	        'key'    => $prefix . $file_name
        ) );
	...
	function get_attachment_s3_info( $post_id ) {
		return get_post_meta( $post_id, 'dreamspeed_info', true );
	}
	...
	function handle_post_request() {
		...
		$post_vars = array( 'bucket', 'virtual-host', 'expires', 'permissions', 'cloudfront', 'object-prefix', 'copy-to-dreamspeed', 'serve-from-s3', 'remove-local-file', 'force-ssl', 'hidpi-images', 'object-versioning' );
