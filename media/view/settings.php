...
<div class="wrap dreamspeed-main">
	...
	<tr><th scope="row">Options</th>
		<td>
			...
			<label for="serve-from-s3"> <?php _e( 'Point file URLs to DreamSpeed/DNS Alias for files that have been copied to S3 (recommended)', 'dreamspeed-cdn' ); ?></label>
			<br />
			<input type="checkbox" name="fullspeed" value="1" id="fullspeed" <?php echo $this->get_setting( 'fullspeed' ) ? 'checked="checked" ' : ''; ?> />
			<label for="fullspeed"> <?php _e( 'Serve files from dream.io (recommended; fastest)', 'dreamspeed-cdn' ); ?></label>
			<br />
			<input type="checkbox" name="force-ssl" value="1" id="force-ssl" <?php echo $this->get_setting( 'force-ssl' ) ? 'checked="checked" ' : ''; ?> />
			<label for="force-ssl"> <?php _e( 'Always serve files over https (SSL) (slowest; overrides CDN alias)', 'dreamspeed-cdn' ); ?></label>
			<br />
			<input type="checkbox" name="expires" value="1" id="expires" <?php echo $this->get_setting( 'expires' ) ? 'checked="checked" ' : ''; ?> />
			<label for="expires"> <?php printf( __( 'Set a <a href="%s" target="_blank">far future HTTP expiration header</a> for uploaded files (recommended)', 'dreamspeed-cdn' ), 'http://developer.yahoo.com/performance/rules.html#expires' ); ?></label>
		</td>
	</tr>
